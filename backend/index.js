...
app.get(`/GetProduct/`, (req, res) => {
    if (req.query.urlpath == "/") {
        if (req.query.cateId == 0) {
            client.query(`SELECT * from "Product" INNER JOIN "Category" ON "Product"."CateId" = "Category"."CateId" WHERE "Product"."ProdIsActive" = true`, (err, result) => {
                if (err) {
                    console.log(err)
                } else {
            ...
            client.query(`SELECT * from "Product" INNER JOIN "Category" ON "Product"."CateId" = "Category"."CateId" WHERE "Product"."CateId" = ${req.query.cateId} AND "Product"."ProdIsActive" = true ORDER BY "Product"."ProdId"`, (err, result) => {
                if (err) {
                    console.log(err)
                } else {
                    res.send(result.rows)
        ...
        if (req.query.activefilter != "-" && req.query.cateId != "0") {
            client.query(`SELECT * from "Product" INNER JOIN "Category" ON "Product"."CateId" = "Category"."CateId" WHERE "Product"."ProdIsActive" = '${req.query.activefilter}' AND "Product"."CateId" = ${req.query.cateId}`, (err, result) => {
                if (err) {
                    console.log(err)
                } else {
                    res.send(result.rows)
...
app.put(`/UpdateCategoryDetail/`,(req,res) =>{
    ...
    "CateIsActive" = ${req.body.editCateIsActive}
...
app.put(`/UpdateProductDetail/`,(req,res) =>{
    ...
    "ProdIsActive" = '${req.body.editIsActive}'
